...
for f in files:
    ...
    fig.savefig(f'results/tests/individual_lstm/{f}/forecast_plots3_layers.png')
    plt.close()
    ...
    with open(f'results/tests/multifit/{f}/error_metrics_total_3_layers.csv', 'w') as error_file:
...
for f in files:
    ...
    fig.legend(handles=[l1, l2], labels=[r'$y$', r'$\hat{y}$'], loc=7, borderaxespad=0.1)
    fig.savefig(f'results/tests/individual_lstm/{f}/forecast_plots_3_layers_l2.png')
    plt.close()
    ...
    mpe = mean_absolute_percentage_error(true_array, predictions_array)
    metrics = {'Mean Squared Error': mse, 'Mean Absolute Error': mae, 'Mean Absolute Percentage Error': mpe}
    with open(f'results/tests/multifit/{f}/error_metrics_total_3_layers_l2.csv', 'w') as error_file:
        w = csv.writer(error_file)
        for key, value in metrics.items():
            w.writerow([key, value])
