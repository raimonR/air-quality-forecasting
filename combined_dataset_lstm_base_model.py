...
for i, (l1, l2) in enumerate(l1l2_weights):
    ...
    for j in range(repeats):
        ...
        model.add(Bidirectional(LSTM(units=64, return_sequences=True, activity_regularizer=l1_l2(l1, l2)))) # , recurrent_regularizer=l1_l2(l1, l2))))
        model.add(Bidirectional(LSTM(units=32, activity_regularizer=l1_l2(l1, l2)))) # , recurrent_regularizer=l1_l2(l1, l2))))
...
ax.set(title='Activity Regularizer', yscale='log')
fig = ax.get_figure()
fig.savefig('results/tuning/combined_lstm/activity_regularizer.png')
plt.close()
