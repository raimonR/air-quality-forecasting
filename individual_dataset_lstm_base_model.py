...
# model.add(Bidirectional(LSTM(units=128, return_sequences=True, kernel_regularizer=l1_l2(l1l2[0], l1l2[1]))))
...
print('done with 64-32 node neural network')
...
model = Sequential()
model.add(Input(shape=(past, n_features)))
model.add(Bidirectional(LSTM(units=256, return_sequences=True, activity_regularizer=l1_l2(l1l2[0], l1l2[1]))))
# model.add(Bidirectional(LSTM(units=128, return_sequences=True, kernel_regularizer=l1_l2(l1l2[0], l1l2[1]))))
model.add(Bidirectional(LSTM(units=128, kernel_regularizer=l1_l2(l1l2[0], l1l2[1]))))
model.add(Dense(units=horizon))
model.summary()
...
for idx, f in enumerate(files):
    ...
    mpe = mean_absolute_percentage_error(true_array, predictions_array)
    metrics = {'Mean Squared Error': mse, 'Mean Absolute Error': mae, 'Mean Absolute Percentage Error': mpe}
    with open(f'results/tests/individual_lstm/{f}/error_metrics_total_256_node.csv', 'w') as error_file:
        w = csv.writer(error_file)
        for key, value in metrics.items():
            w.writerow([key, value])
    ...
    fig.legend([l1, l2], labels=[r'$y$', r'$\hat{y}$'], loc=7, borderaxespad=0.1)
    fig.savefig(f'results/tests/individual_lstm/{f}/forecast_plots_256_node.png')
    plt.close()

    keras.backend.clear_session()
    print(f'done with {f}')

print('done with 256-128 node neural network')
